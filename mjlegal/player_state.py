...
class PlayerState:
    ...
    def tsumo(self, pai):
        ...
        rinshan = (
            ...
            else self.prev_action.type in (ActionType.ANKAN, ActionType.KAKAN, ActionType.DAIMINKAN, ActionType.NUKI)
        )
    ...
    def pon(self, pai, consumed, from_id):
        ...
        self.prev_action = Action(
            type=ActionType.PON, actor=self.player_id, target=from_id, tile=tile, consumed=consumed_tiles
        )
        return self.prev_action
    ...
    def chi(self, pai, consumed, from_id):
        ...
        self.melds.append(meld)
        self.prev_action = Action(
            type=ActionType.CHI, actor=self.player_id, target=from_id, tile=tile, consumed=consumed_tiles
        )
        return self.prev_action
    ...
    def kakan(self, pai):
        ...
        pon_idx = [
            ...
            if meld.type == Meld.PON and Tile.equal(tile, meld.tiles[0], True)
        ][0]
    ...
    def daiminkan(self, pai, consumed, from_id):
        ...
        self.melds.append(meld)
        self.prev_action = Action(
            type=ActionType.DAIMINKAN, actor=self.player_id, target=from_id, tile=tile, consumed=consumed_tiles
        )
        return self.prev_action
    ...
    def export_tile136(self):
        return {"tiles": TilesUtil.tiles_to_str(self._tiles), "melds": self.melds136}
