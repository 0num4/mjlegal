...
class PossibleActionGenerator:
    ...
    def possible_game_actions(self, game_state):
        actions = (
            self.possible_actions_hora(game_state)
        ...
        if previous_action is not None:
            ...
            if prev_type in (ActionType.TSUMO, ActionType.PON, ActionType.CHI):
                ...
                if prev_actor_state.is_reach and prev_type == ActionType.TSUMO:
                    tsumogiri_action = tsumogiri_action = Action(
                        type=ActionType.DAHAI, actor=prev_actor, tile=prev_actor_state.tsumo_tile, tsumogiri=True
                    )
                ...
                for id in other_player_ids:
                    ...
                    if not player_state.is_reach:
                        ...
                        for consumed in consumed_list:
                            pon_action = Action(
                                type=ActionType.PON, actor=id, target=prev_actor, consumed=consumed, tile=prev_dahai
                            )
        ...
        if previous_action is not None and game_state.num_pipais > 0:
            ...
            if prev_type == ActionType.TSUMO:
                ...
                for ankan_t in ankan_tiles:
                    ...
                    kan_action = Action(type=ActionType.ANKAN, actor=prev_actor, consumed=consumed)
                ...
                for meld in melds:
                    if meld.type == Meld.PON:
                        ...
                        if len(kakan_tiles) > 0:
                            kan_action = Action(
                                type=ActionType.KAKAN, actor=prev_actor, tile=kakan_tiles[0], consumed=meld.tiles
                            )
                            actions.append(kan_action)
        return actions
    ...
    def possible_actions_dahai_with_reach(self, game_state):
        ...
        if previous_action is not None:
            ...
            if prev_type == ActionType.TSUMO:
                ...
                prev_actor_state = game_state.player_states[prev_actor]
                if (
                    not (prev_actor_state.is_reach)
                    and prev_actor_state.is_menzen
                    and prev_actor_state.score >= 1000
                    and game_state.num_pipais >= game_state.num_players
                ):
                    tiles = prev_actor_state.tehai
        ...
        if previous_action is not None:
            ...
            for actor in actors:
                if self.hand_tool.can_hora(game_state, actor, previous_action.tile, is_tsumo, is_rinshan):
                    hora_action = Action(type=ActionType.HORA, actor=actor, target=target, tile=previous_action.tile)
